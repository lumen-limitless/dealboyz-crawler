...
export async function amazonHandler(
    ...
    const { upc, retailer, title } = request.userData;
    log.info(`Processing Amazon page for product: ${title} (UPC: ${upc})`);
    ...
    try {
        ...
        if (noResults.length > 0) {
            log.info(`No results found on Amazon for product: ${title}`);
            return;
        }
        ...
        if (!firstProduct) {
            log.info(`No valid product found on Amazon for product: ${title}`);
            return;
        }
        ...
        if (!productUrl) {
            log.warning(`Could not extract product URL for product: ${title}`);
            return;
        }
        ...
        if (!priceText) {
            log.warning(`Could not extract price for Amazon product: ${title}`);
            return;
        }
        ...
        if (!priceMatch) {
            log.warning(`Could not parse price text: ${priceText} for product: ${title}`);
            return;
        }
        ...
        log.info(`Successfully extracted Amazon price for ${title}: $${price.toFixed(2)}`);
        
    } catch (error) {
        log.error(`Error processing Amazon page for product ${title}:`, { error: String(error) });
    }
}
